...
#             self._target.set_mocap_pose(self._physics, position=[0.7, 0.3, 0.02], quaternion=[0, 0, 0, 1])
...
#         reward = -position_distance #-total_distance
...
class UR5eGrpEnvROS2(Env):
    ...
    def __init__(self, render_mode=None):
        ...
        # Define action and observation spaces
        self.observation_space = spaces.Box(
            low=-np.inf, high=np.inf, shape=(13,), dtype=np.float64  # joint pos, velocities, and distance to target
        )
        self.action_space = spaces.Box(
            low=-0.05, high=0.05, shape=(6,), dtype=np.float64  
        )
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        ee_pos = self._physics.bind(self._arm.eef_site).xpos
        ee_xmat = self._physics.bind(self._arm.eef_site).xmat
        ee_xquat = mat2quat(ee_xmat.reshape(3, 3))
    ...
    def reset(self, seed=None, options=None, target_position=None):
        ...
        with self._physics.reset_context():
            ...
            # Use provided target position or generate a random one
            if target_position is None:
                target_position = np.random.uniform(low=[0.4, -0.2, 0.2], high=[0.6, 0.2, 0.4])

            # Set mocap pose to new target
            self._target.set_mocap_pose(self._physics, position=target_position, quaternion=[0, 0, 0, 1])
    ...
    def step(self, action=None):
        ...
        # Get observations and reward
        observation = self._get_obs()
        reward, terminated = self._compute_reward_and_done(observation[-1])
    ...
    def _compute_reward_and_done(self, distance_to_target):
        reward = -distance_to_target  # Closer is better
        terminated = distance_to_target < 0.02  # Threshold for reaching the target
        if terminated:
